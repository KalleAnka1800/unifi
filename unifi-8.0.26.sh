...
add_mongodb_repo() {
  ...
  if [[ -n "${mongodb_version_major_minor}" ]]; then
    if ! [[ -e "/usr/share/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" ]] || [[ "${mongodb_key_update}" == 'true' ]]; then
      echo -e "${WHITE_R}#${RESET} Adding key for MongoDB ${mongodb_version_major_minor}..."
      aptkey_depreciated
      if [[ "${apt_key_deprecated}" == 'true' ]]; then
        if wget -qO - "${repo_http_https}://pgp.mongodb.com/server-${mongodb_version_major_minor}.asc" | gpg --dearmor | tee -a "/usr/share/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" &> /dev/null; then
          ...
          if wget -qO - "${repo_http_https}://www.mongodb.org/static/pgp/server-${mongodb_version_major_minor}.asc" | gpg --dearmor | tee -a "/usr/share/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" &> /dev/null; then
  ...
  if [[ -z "${trusted_mongodb_repo}" ]]; then
    if grep -qio 'expkeysig.*mongo' /tmp/EUS/keys/apt_update; then
      echo -e "${YELLOW}#${RESET} Looks like the key for the MongoDB Repository is expired..."
      echo -e "${WHITE_R}#${RESET} Adding the repository with an extra trusted flag...\\n"
      trusted_mongodb_repo=" trusted=yes"
      if [[ "${mongodb_expire_attempt}" != 'true' ]]; then mongodb_expire_attempt="true"; add_mongodb_repo; fi
    fi
  fi
}
...
if grep -qio 'expkeysig.*mongo' /tmp/EUS/keys/apt_update; then
  echo -e "${YELLOW}#${RESET} The script detected an expired MongoDB repository key... We'll attempt to update it...\\n"
...
libssl_installation() {
  if [[ "${libssl_install_required}" == 'true' ]]; then
    ...
    if [[ "${libssl_package_empty}" != 'false' ]]; then
      ...
      if ! [[ -e "${eus_dir}/logs/libssl-failure-debug-info.json" ]] || ! [[ -s "${eus_dir}/logs/libssl-failure-debug-info.json" ]] || ! jq empty "${eus_dir}/logs/libssl-failure-debug-info.json"; then
        jq -n \
...
mongodb_installation_armhf() {
  aptkey_depreciated
  if [[ "${apt_key_deprecated}" == 'true' ]]; then
    if wget -qO - "${raspbian_repo_url}.public.key" | gpg --dearmor | tee -a "/usr/share/keyrings/raspbian.gpg" &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added key for the raspbian repository! \\n"; signed_by_value_raspbian="[ signed-by=/usr/share/keyrings/raspbian.gpg ] "; else echo -e "${RED}#${RESET} Failed to add the key for the raspbian repository...\\n"; abort; fi
  else
  ...
  if dpkg -l mongodb-org-server 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
    ...
    if [[ "${mongodb_org_version_no_dots::2}" == '44' && "$(echo "${mongodb_org_version}" | cut -d'.' -f3)" -gt "18" ]]; then
      ...
      while read -r mongodb_package; do
        ...
        if DEBIAN_FRONTEND='noninteractive' apt-get -y --allow-downgrades "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install "${mongodb_package}${arm64_mongodb_version}" &>> "${eus_dir}/logs/arm64-mongodb-downgrade.log"; then
          echo -e "${GREEN}#${RESET} Successfully downgraded ${mongodb_package} to version ${arm64_mongodb_version}! \\n"
        else
          echo -e "${RED}#${RESET} Failed to downgrade ${mongodb_package} to version ${arm64_mongodb_version}...\\n"
          abort
        fi
        echo -e "${WHITE_R}#${RESET} Preventing ${mongodb_package} from upgrading..."
        if echo "${mongodb_package} hold" | dpkg --set-selections; then
